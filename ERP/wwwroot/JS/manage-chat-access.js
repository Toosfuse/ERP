...
function loadUserData(userId) {
    $.get('/Chat/GetUserInfo', { userId: userId }, function(user) {
        $('#selectedUserNameHeader').text(user.name);
    });
...
function renderAvailableUsers() {
    ...
    allAvailableUsers.forEach(function(user) {
        html += `
            <div class="user-card available-card" data-user-id="${user.id}">
                ...
                <button class="btn-add" data-user-id="${user.id}">
    ...
    $('#availableUsersList').html(html || '<div class="empty-state"><i class="fa fa-check-circle"></i><p>همه کاربران اضافه شده‌اند</p></div>');
}
...
function renderAllowedUsers() {
    ...
    allowedUsers.forEach(function(user) {
        html += `
            <div class="user-card allowed-card" data-user-id="${user.id}">
                <div class="user-info">
                    <img src="${user.image}" alt="${user.name}" class="user-avatar" />
                    <span class="user-name">${user.name}</span>
                </div>
                <button class="btn-remove" data-user-id="${user.id}">
    ...
    $('#allowedUsersList').html(html || '<div class="empty-state"><i class="fa fa-user-times"></i><p>هیچ کاربری اضافه نشده است</p></div>');
}

function updateCounts() {
    $('#availableCount').text(allAvailableUsers.length);
    $('#allowedCount').text(allowedUsers.length);
}
